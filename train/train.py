...
OUTPUT_FILE = os.path.join(LOG_DIR,
                           'results_train_eval_200/')
if not os.path.exists(OUTPUT_FILE):
    os.mkdir(OUTPUT_FILE)


# Load Frustum Datasets. Use default data paths.
TRAIN_DATASET = provider.FrustumDataset('pc_radar_2',database="KITTI" ,npoints=NUM_POINT, split='train',
...
EVAL_DATASET = provider.FrustumDataset('pc_radar_2',database="KITTI",npoints=NUM_POINT, split='val',rotate_to_center=False, one_hot=True,all_batches = True, translate_radar_center=False, store_data=True, proposals_3 =True ,no_color=True)
TEST_DATASET = provider.FrustumDataset('pc_radar_2',database="KITTI_2",npoints=NUM_POINT, split='test',rotate_to_center=False, one_hot=True,all_batches = True, translate_radar_center=False, store_data=True, proposals_3 =True ,no_color=True)
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        eval_writer = tf.summary.FileWriter(os.path.join(LOG_DIR, 'val'), sess.graph)
...
def eval_one_epoch(sess, ops, test_writer,dataset,database,split):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_seg_mask = np.argmax(logits_val, 2)
    ...
    if(EPOCH_CNT==40):
        IOU3d, GT_box_list, pred_box_list = compare_box_iou(dataset.id_list, dataset.indice_box,
                                                            ...
                                                            heading_cls_list,
                                                            center_list,
                                                            segp_list, seg_list,split)
        accuracy_5,recall_5,accuracy_4,recall_4,accuracy_35,recall_35,accuracy_3,recall_3,accuracy_25,recall_25=eval_per_frame(dataset.id_list, dataset.indice_box, ps_list, seg_list, segp_list, GT_box_list,
                       pred_box_list, IOU3d, score_list,database,split)

        write_detection_results_test(split, dataset.id_list,
                                     center_list,
                                     heading_cls_list, heading_res_list,
                                     size_cls_list, size_res_list, rot_angle_list, segp_list)
        log_string(split + ' accuracy_5: %f' % \
                   accuracy_5)
        log_string(split + ' recall_5: %f' % \
                   recall_5)
        log_string(split + ' accuracy_4: %f' % \
                   accuracy_4)
        log_string(split + ' recall_4: %f' % \
                   recall_4)
        log_string(split + ' accuracy_35: %f' % \
                   accuracy_35)
        log_string(split + ' recall_35: %f' % \
                   recall_35)
        log_string(split + ' accuracy_3: %f' % \
                   accuracy_3)
        log_string(split + ' recall_3: %f' % \
                   recall_3)
        log_string(split + ' accuracy_25: %f' % \
                   accuracy_25)
        log_string(split + ' recall_25: %f' % \
                   recall_25)



def compare_box_iou(id_list,indice_box,size_residual_GT,size_class_GT,heading_res_GT,heading_class_GT,center_GT,
                    score_list,size_res_list,size_cls_list,heading_res_list, heading_cls_list,center_list,segp_list,seg_list,split):

    file1 = open(OUTPUT_FILE+"/results_"+split+".txt" , "w")
    IoU=[]
    GT_box_list=[]
    pred_box_list=[]
...
def eval_per_frame(id_list,indice_box_list ,ps_list, seg_list, segp_list,GT_box_list,pred_box_list,IOU3d,score_list,database,split):
    ...
    accuracy_5,recall_5,accuracy_4,recall_4,accuracy_35,recall_35,accuracy_3,recall_3,accuracy_25,recall_25=precision_recall(id_list_frame,pred_box_frame,corners_GT_frame,score_frame,IoU_frame,indice_box_frame,id_list_GT)
    return accuracy_5,recall_5,accuracy_4,recall_4,accuracy_35,recall_35,accuracy_3,recall_3,accuracy_25,recall_25
...
def write_detection_results_test(split, id_list, center_list, \
                                 heading_cls_list, heading_res_list, \
