...
g_type_mean_size = {'Car': np.array([3.88311640418,1.62856739989,1.52563191462]),
                    ...
                    'Pedestrian': np.array([1.822122,1.4857313,1.407308]),#'Pedestrian': np.array([0.84422524,0.66068622,1.76255119]),
...
def get_loss_batch(mask_label,mask_batch, center_label, \
    ...
    heading_class_loss = tf.reduce_sum( \
        ...
        logits=end_points['heading_scores'], labels=heading_class_label)*mask_batch)/tf.maximum(tf.reduce_sum(mask_batch),1.0)
    ...
    size_class_loss = tf.reduce_sum( \
        tf.nn.sparse_softmax_cross_entropy_with_logits( \
        logits=end_points['size_scores'], labels=size_class_label)*mask_batch)/ tf.maximum(tf.reduce_sum(mask_batch),1.0)
